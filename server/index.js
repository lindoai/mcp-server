...
server.registerTool(
  ...
  {
    title: "Edit Website",
    description: "Edit an existing website (across its pages) using AI. Starts an asynchronous workflow and returns a `workflow_id` immediately — poll `check_edit_status` with that id to track progress and get the final result.",
    inputSchema: {
    website_id: z.string().describe("The website ID"),
    prompt: z.string().describe("Describe the change to make to the website"),
    publish: z.boolean().optional().describe("When true, publish the edited page(s) to the live website. When false/omitted, save the changes as drafts to review + publish from the editor."),
    schedule_at: z.string().optional().describe("Optional ISO 8601 datetime to schedule for later"),
  },
    annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: true },
  },
  async ({ website_id, prompt, publish, schedule_at }) => {
    const body = { prompt };
    if (typeof publish === "boolean") body.publish = publish;
    if (schedule_at) body.schedule_at = schedule_at;
    const data = await apiCall(`/v1/ai/workspace/website/${website_id}/edit`, "POST", body);
    const payload = data?.result || data;
    return { content: [{ type: "text", text: JSON.stringify(payload, null, 2) }] };
  }
);
...
server.registerTool(
  "update_page",
  {
    ...
    seo: z.record(z.unknown()).optional().describe("SEO settings object, including canonical_url (absolute URL; empty string clears the override)"),
  },
    annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: false },
  },
...
server.registerTool(
  "publish_page",
  {
    ...
    html: z.string().describe("Page content in HTML"),
    page_title: z.string().describe("Page title for SEO"),
  },
    annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: true },
  },
  async ({ website_id, path, html, page_title }) => {
    const body = {
      path,
      html,
      seo: { page_title },
...
server.registerTool(
  "update_page_content",
  {
    ...
    website_id: z.string().describe("The website ID"),
    page_id: z.string().describe("The page ID"),
    html: z.string().describe("Updated page content in HTML"),
    seo: z.record(z.unknown()).optional().describe("SEO settings object, including canonical_url (absolute URL; empty string clears the override)"),
  },
    annotations: { readOnlyHint: false, destructiveHint: false, openWorldHint: true },
  },
  async ({ website_id, page_id, html, seo }) => {
    const body = { html };
    if (seo) body.seo = seo;
...
server.registerTool(
  "check_edit_status",
  {
    title: "Check Website Edit Status",
    description: "Poll the status of a website-edit workflow started by `edit_website`. Pass the `workflow_id` you received from that tool. While running, call this again after `poll_after_ms` ms. Once `done` is true, `status` is `complete` or `errored`; when complete, `result` holds the changed pages.",
    inputSchema: {
    workflow_id: z.string().describe("Workflow id returned by edit_website"),
  },
    annotations: { readOnlyHint: true, destructiveHint: false, openWorldHint: false },
  },
  async ({ workflow_id }) => {
    const data = await apiCall(`/v1/ai/workspace/website/edit/status/${encodeURIComponent(workflow_id)}`, "GET");
    const payload = data?.result || data;
    return { content: [{ type: "text", text: JSON.stringify(payload, null, 2) }] };
  }
);
